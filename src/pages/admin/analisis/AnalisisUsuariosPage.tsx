...
const AnalisisUsuariosPage = () => {
  ...
  const [filtroTipo, setFiltroTipo] = useState(''); // 'ciudadano' | 'institucion' | ''
  ...
  const resetFiltros = () => {
    setMesDesde(''); setMesHasta(''); setFiltroTipo(''); setFiltroRol('');
  };
  ...
  const rolAreaData = useMemo(() => {
    ...
    const rows = (stats.por_mes_rol ?? [])
      .filter(d => inRange(d.mes, mesDesde, mesHasta) && (!filtroRol || d.rol === filtroRol));
    const map: Record<string, Record<string, number>> = {};
  ...
  /* pie re-agregado cuando hay filtro de fecha */
  const rolPieData = useMemo(() => {
    ...
    if (mesDesde || mesHasta) {
      ...
      (stats.por_mes_rol ?? [])
        .filter(r => inRange(r.mes, mesDesde, mesHasta))
  ...
  const totalPeriodo = useMemo(
    () => mesData.reduce((s, d) => s + d.count, 0),
    [mesData],
  );
  ...
  const regionData = (stats.por_region ?? []).filter(r => r.region).slice(0, 10)
    .map(r => ({ region: r.region || 'Sin región', count: r.count }));
  const comunaData = (stats.top_comunas ?? []).filter(c => c.comuna).slice(0, 10)
  ...
  return (
    <div className="min-h-screen flex flex-col admin-glass">
      ...
      <main className="flex-1 py-6 px-6 lg:px-8 space-y-6">
        ...
        {/* Región y comunas (datos totales — no varían con filtro de fecha) */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <ChartCard title="Usuarios por región">
            {regionData.length === 0 ? (
          ...
          <ChartCard title="Top 10 comunas">
            {comunaData.length === 0 ? (
